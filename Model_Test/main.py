...
def main():
   ...
   # Get the actual UUID of the USDC wallet
    ...
    #     print(f"{wallet['currency']} - {wallet['uuid']} - Balance: {wallet['available_balance']['value']}")
    #     if wallet['currency'] == "USDC":
    #         wallet_id_usdc = wallet['uuid']
    #         break
    ...
    for spot_product in spot_products["products"]:
        ...
        if product_id.endswith(target_currency) and float(price_change) > MIN_PRICE_CHANGE_24_HRS and not product_id in products_being_selled:
    ...
    if number_of_products_to_buy == 0:
        ...
        print(f"MIN_PRICE_CHANGE_24_HRS: {MIN_PRICE_CHANGE_24_HRS}")
        return
